...
func (repositorio Usuarios) BuscarPorID(usuairoID uint64) (modelos.Usuario, error) {
	linhas, erro := repositorio.db.Query(
		"select id, nome, nick, email, criadoEm from usuarios where id = ?", usuairoID,
	)
...
func (repositorio Usuarios) BuscarPorEmail(email string) (modelos.Usuario, error) {
	linhas, erro := repositorio.db.Query("select id, senha from usuarios where id = ? ", email)
	if erro != nil {
		return modelos.Usuario{}, erro
	}
	defer linhas.Close()
